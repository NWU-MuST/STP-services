...
SPECIAL_DICT_FILE = os.path.join(TEXTNORM_ROOT_DIR, "data", "nsoZA", "g2p.specialdict.txt")
ABBREVS_FILE = os.path.join(TEXTNORM_ROOT_DIR, "data", "nsoZA", "norm.abbrev.json")
VALID_GRAPHS_FILE = os.path.join(TEXTNORM_ROOT_DIR, "data", "nsoZA", "norm.graphset.txt")

def pref_engZA(s):
    if not s.startswith("engza_"):
        return "engza_"+s
    else:
        return s
...
UNK = "unk_unk"
#Punctuation expected around tokens and lowercase Northern Sotho graphemes
VALID_PUNCTS = '".,:;!?(){}[]-'
...
def expand_dates(m):
    ...
    return [[pref_engZA(w) for w in e] for e in expansions]
...
def expand_crds(m):
    ...
    return [[pref_engZA(w) for w in e] for e in [t.split() for t in num_expand(int(tokentext))]]
...
def expand_crdsuff(m):
    ...
    #DEMIT
    return [[pref_engZA(w) for w in e] for e in [t.split() for t in num_expand(int(digits))]]
...
def expand_ords(m):
    ...
    return [[pref_engZA(w) for w in e] for e in [ord_expand(int(tokentext)).split()]]
...
def expand_floats(m):
    ...
    return [[pref_engZA(ww) for ww in w]]
