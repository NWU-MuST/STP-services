...
cat $g_fst |  utils/eps2disambig.pl | utils/s2eps.pl | fstcompile --isymbols=$lang/words.txt \
      --osymbols=$lang/words.txt  --keep_isymbols=false --keep_osymbols=false | \
     fstrmepsilon | fstarcsort --sort_type=ilabel > $tmpdir/G.fst
...
# place of -o
  fsttablecompose $lang/L.fst $tmpdir/G.fst | fstdeterminizestar --use-log=true | \
  ...
  if $reverse; then
    ...
    make-h-transducer --transition-scale=$tscale $tmpdir/ilabels_${N}_${P} $tree $model \
       > $tmpdir/Ha.fst  || exit 1;
  fi
