...
fstcomposecontext --context-size=$N --central-position=$P $tmpdir/ilabels_${N}_${P} < $tmpdir/LG.fst | \
    fstarcsort --sort_type=ilabel > $clg
fstisstochastic $clg || echo "[info]: CLG not stochastic."

if $reverse; then
    make-h-transducer --reverse=true --push_weights=true --disambig-syms-out=$dir/disambig_tid.int --transition-scale=$tscale $tmpdir/ilabels_${N}_${P} $tree $model > $tmpdir/Ha.fst  || exit 1;
else
    make-h-transducer --transition-scale=$tscale $tmpdir/ilabels_${N}_${P} $tree $model > $tmpdir/Ha.fst  || exit 1;
fi
...
add-self-loops --self-loop-scale=0 --reorder=true $model < $tmpdir/HCLGa.fst > $outdir/HCLG.fst || exit 1;
