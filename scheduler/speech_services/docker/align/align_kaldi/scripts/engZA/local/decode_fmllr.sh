...
if [ $stage -le 2 ]; then
  ...
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode.JOB.log \
    ...
    $adapt_model $graph "$pass1feats" "ark:|gzip -c > $dir/lat.tmp.JOB.gz" \
    || exit 1;
fi
##
