...
if [ $# != 4 ]; then
   ...
   echo "Usage: steps/decode_fmllr.sh [options] <graph> <graph-dir> <data-dir> <decode-dir>"
...
for f in $data/feats.scp $srcdir/tree; do
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ -f "$graphdir/num_pdfs" ]; then
    ...
    local/decode_must.sh --acwt $acwt --nj $nj --cmd "$cmd" --beam $first_beam --model $alignment_model\
      --max-active $first_max_active --num-threads $num_threads\
      --skip-scoring true $graph $graphdir $data $si_dir || exit 1;
  fi
fi
##
...
if [ $stage -le 2 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode1.JOB.log\
    ...
    --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$wst \
    $adapt_model $graph "$pass1feats" "ark:|gzip -c > $dir/lat1.JOB.gz" \
    || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode2.JOB.log\
    ...
    --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$wst \
    $adapt_model $graph "$pass2feats" "ark:|gzip -c > $dir/lat2.JOB.gz" \
    || exit 1;
fi
