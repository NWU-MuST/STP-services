...
if [ $# -ne 4 ]; then
  echo "Usage: steps/decode_sgmm2.sh [options] <graph> <graph-dir> <data-dir> <decode-dir>"
...
for f in $graph $data/feats.scp $srcdir/final.mdl; do
...
if [ $stage -le 2 ]; then
  ...
  $cmd $parallel_opts JOB=1:$nj $dir/log/decode_pass1.JOB.log \
    ...
    --word-symbol-table=$wst --max-mem=$max_mem "$gselect_opt_1stpass" $alignment_model \
    $graph "$feats" "ark:|gzip -c > $dir/pre_lat.JOB.gz" || exit 1;
fi
