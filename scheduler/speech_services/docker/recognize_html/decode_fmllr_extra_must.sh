...
if [ $# != 3 ]; then
   ...
   echo "  --source-dir <model-dir>                 # location of models, transforms, etc ..."
   exit 1;
fi
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ -f "$graphdir/num_pdfs" ]; then
    ...
    decode_must.sh --acwt $acwt --nj $nj --cmd "$cmd" --beam $first_beam --model $alignment_model\
      --max-active $first_max_active --num-threads $num_threads\
      --skip-scoring true --source-dir $source_dir $graphdir $data $si_dir || exit 1;
  fi
fi
##
